...
// TEMPORARY: returns all generated poster art at once as { name: base64 },
// so the whole set can be pulled in a single request. Delete with the
// [name] route once posters are committed.
...
export async function GET() {
  ...
  await Promise.all(
    Object.entries(MAP).map(async ([name, file]) => {
      ...
      if (r.ok) out[name] = Buffer.from(await r.arrayBuffer()).toString('base64');
    }),
  );
